...
このアプリは認証機能がなく、学校の課題として自分ひとりで動作確認するための個人利用が前提で、誰かに使ってもらうことは想定していない。そのため、今回の構成は「無料利用枠の範囲に収まること」を最優先にし、費用が発生しうる要素は使わない。

| 項目 | 今回の構成 | 無料利用枠との関係 |
|------|-----------|---------------------|
| サーバー | EC2 `t3.micro` を1台のみ | 対象インスタンスタイプを750時間／月まで無料（1台を常時稼働させても月の時間数以内） |
| ディスク | ルートボリューム8GB（`root_block_device` で明示指定） | EBS 30GB-月まで無料。8GBなら余裕を持って収まる |
...
> **注意 — 無料利用枠には期限がある**
> ここでいう無料利用枠（EC2やEBSなど）の多くは、**AWSアカウントを作成してから12か月間限定**のもの。作成から12か月を過ぎたアカウントでは対象外になり、`t3.micro`でも実費がかかる（東京リージョンで概算、稼働させっぱなしでも月1,000円未満程度）。自分のアカウントがいつ作成されたかは、コンソール右上のアカウント名メニュー →「請求ダッシュボード」→「無料利用枠」で確認できる。心配な場合は[9. 後片付けとコスト管理](#9-後片付けとコスト管理)の予算アラートを必ず設定し、使わない時間は`terraform destroy`で消しておくとよい。
...
variable "root_volume_size_gb" {
  default = 8 # 無料利用枠(30GB-月まで無料)に十分収まるサイズ
}
